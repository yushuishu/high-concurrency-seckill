...
public class SeckillController {
    ...
    public ApiResponse<String> startLock(long skgId) {
        try {
            ...
            ApiResponse<String> apiResponse = seckillService.startSecondKillByLock(skgId, userId);
            if(apiResponse != null){
    ...
    public ApiResponse<String> startLock2(long skgId) {
        ...
        try {
            log.info("开始秒杀方式一...");
            final long userId = (int) (new Random().nextDouble() * (99999 - 10000 + 1)) + 10000;
            ApiResponse<String> apiResponse = seckillService.startSecondKillByLock2(skgId, userId);
            if(apiResponse != null){
                log.info("用户:{}--{}", userId, apiResponse.getMessage());
            }else{
                log.info("用户:{}--{}", userId, "人也太多了，请稍后！");
    ...
    public ApiResponse<String> startAop(long skgId) {
        ...
        try {
            log.info("开始秒杀方式二...");
            final long userId = (int) (new Random().nextDouble() * (99999 - 10000 + 1)) + 10000;
            ApiResponse<String> apiResponse = seckillService.startSecondKillByAop(skgId, userId);
            if(apiResponse != null){
                log.info("用户:{}--{}", userId, apiResponse.getMessage());
            }else{
                log.info("用户:{}--{}", userId, "人也太多了，请稍后！");
    ...
    public ApiResponse<String> startPesLockOne(long skgId) {
        try {
            log.info("开始秒杀方式三...");
            final long userId = (int) (new Random().nextDouble() * (99999 - 10000 + 1)) + 10000;
            ApiResponse<String> apiResponse = seckillService.startSecondKillByUpdate(skgId, userId);
            if(apiResponse != null){
                log.info("用户:{}--{}", userId, apiResponse.getMessage());
            }else{
                log.info("用户:{}--{}", userId, "人也太多了，请稍后！");
    ...
    public ApiResponse<String> startPesLockTwo(long skgId) {
        try {
            log.info("开始秒杀方式四...");
            final long userId = (int) (new Random().nextDouble() * (99999 - 10000 + 1)) + 10000;
            ApiResponse<String> apiResponse = seckillService.startPesLockTwo(skgId, userId);
            if(apiResponse != null){
                log.info("用户:{}--{}", userId, apiResponse.getMessage());
            }else{
                log.info("用户:{}--{}", userId, "人也太多了，请稍后！");
    ...
    public ApiResponse<String> startOptLock(long skgId) {
        try {
            log.info("开始秒杀方式四...");
            final long userId = (int) (new Random().nextDouble() * (99999 - 10000 + 1)) + 10000;
            ApiResponse<String> apiResponse = seckillService.startSecondKillByPesLock(skgId, userId, 1);
            if(apiResponse != null){
                log.info("用户:{}--{}", userId, apiResponse.getMessage());
            }else{
                log.info("用户:{}--{}", userId, "人也太多了，请稍后！");
